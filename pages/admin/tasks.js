...
Page({
  ...
  createTask() { if(!this.data.title.trim() || !this.data.deadline.trim()) return wx.showToast({title:"请填写任务名称和截止日期",icon:"none"}); const tasks=this.data.tasks.slice(); tasks.unshift({id:Date.now(),assessmentId:this.data.assessmentId,title:this.data.title.trim(),semester:"当前学期",deadline:this.data.deadline.trim(),status:"草稿",completed:false,target:this.data.target}); wx.setStorageSync("assessmentTasks",tasks); this.setData({title:"",deadline:""}); this.onShow() },
