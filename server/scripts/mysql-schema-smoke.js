...
async function main() {
  ...
  const firstId = "smoke-a-" + randomUUID()
  const secondId = "smoke-b-" + randomUUID()
  try {
    ...
    // Re-runnable against populated databases; never persist test semesters.
    await connection.beginTransaction()
    const [current] = await connection.query("SELECT id FROM semesters WHERE status = '当前学期' FOR UPDATE")
    if (!current.length) await connection.query(
